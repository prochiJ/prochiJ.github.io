...
class PortfolioLoader {
  constructor() {
    ...
    // Adjust for root vs subdirectories
    // If path is /portfolio/about/ -> depth might be 2 (portfolio, about)
    // But we need relative path to root.
    // Let's assume we are checking relative to where index.html is.

    // Simple heuristic: count occurrences of '..' needed
    // If we are in /about/index.html, we need ../
    // If we are in /blogs/blog1/index.html, we need ../../

    // Check if we are in a known subdirectory structure
    const path = window.location.pathname;
    let upLevels = '';

    if (path.includes('/blogs/blog')) {
      upLevels = '../../';
    } else if (path.includes('/about/') || path.includes('/contact/') || path.includes('/projects/') || path.includes('/blogs/')) {
      upLevels = '../';
    } else if (path.includes('/pages/')) {
      // Fallback for any unmoved pages
      upLevels = '../';
    }

    this.basePath = upLevels + 'data/';
    this.rootPath = upLevels || './'; // Default to ./ if empty for clearer relative paths, or just empty string? Empty string is better for root.
    // Actually if I use ./ for root, ./projects/ is fine.
    // If I use empty string, projects/ is fine.
    // Let's use empty string if upLevels is empty.
    this.rootPath = upLevels;
    this.inPagesDir = upLevels !== '';
  }
  ...
  generateProjectCard(project) {
    return `
      <a href="${this.adjustLink(project.link)}" class="card flex flex-col gap-6 hover:border-accent/50 transition-all duration-300 cursor-pointer group">
        <figure class="relative overflow-hidden rounded-lg">
          <img src="${this.adjustLink(project.image)}" alt="${project.title}" class="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300">
        </figure>
  ...
  generateBlogCard(blog) {
    ...
    const link = this.adjustLink(blog.link);
  ...
  loadAboutHero() {
    ...
    heroSection.innerHTML = `
      ...
      <div class="card space-y-6">
        <img src="${this.adjustLink(hero.image)}" alt="${hero.imageAlt}" class="w-full rounded-lg grayscale hover:grayscale-0 transition" />
    ...
    section.innerHTML = `
      <div class="card flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex flex-col gap-3 w-full md:w-auto">
          ${cta.buttons.map(btn => `
            <a href="${this.adjustLink(btn.link)}" class="btn ${btn.primary ? 'btn-primary' : ''} text-center">${btn.text}</a>
          `).join('')}
        </div>
      </div>
    `;
  ...
  loadHomeHero() {
    ...
    heroSection.innerHTML = `
      <div>
        ...
        <div class="mt-10 flex flex-wrap gap-4">
          ${hero.buttons.map(btn => `
            <a href="${this.adjustLink(btn.link)}" class="btn ${btn.primary ? 'btn-primary' : ''}">${btn.text}</a>
          `).join('')}
        </div>
      ...
      <div class="card text-center">
        <img src="${this.adjustLink(hero.image)}" alt="${hero.imageAlt}" class="w-full h-auto mx-auto rounded-lg grayscale hover:grayscale-0 transition">
  ...
  loadHomeSections() {
    ...
    if (projectsSection && this.home.projects) {
      ...
      if (cta) {
        ...
        cta.href = this.adjustLink(this.home.projects.ctaLink);
      }
    }
    ...
    if (blogsSection && this.home.blogs) {
      ...
      if (cta) {
        ...
        cta.href = this.adjustLink(this.home.blogs.ctaLink);
      }
    }
    ...
    if (expSection && this.home.experience) {
      ...
      if (cta) {
        ...
        cta.href = this.adjustLink(this.home.experience.ctaLink);
      }
    }
  }
  ...
  loadContactCards() {
    ...
    container.innerHTML = this.contact.contactCards.map(card => `
      <a href="${this.adjustLink(card.link)}" ${card.external ? 'target="_blank" rel="noopener noreferrer"' : ''} class="card text-center space-y-3 p-6 hover:border-accent/50 transition-all duration-300 cursor-pointer group">
  ...
  loadContactNote() {
    ...
    noteCard.innerHTML = `
      <div class="space-y-4 text-center">
        ...
        <div class="pt-2">
          <a href="${this.adjustLink(note.buttonLink)}" class="btn btn-primary inline-block">${note.buttonText}</a>
        </div>
      </div>
    `;
  }
  ...
  loadFooter() {
    ...
    const themes = this.footer.themes;
    ...
    footerElement.innerHTML = `
      <div class="container mx-auto px-6 md:px-16 lg:px-24 flex flex-col gap-8 md:flex-row md:items-start md:justify-between">
        ...
        <div class="grid gap-6 sm:grid-cols-3">
          ...
          <div>
            <h3 class="font-mono text-accent text-sm uppercase tracking-widest">${themes ? themes.heading : 'Themes'}</h3>
            <ul class="mt-3 space-y-2 text-sm">
              ${themes ? themes.links.map(link => `
                <li><a href="${adjustLink(link.url)}" class="nav-link">${link.text}</a></li>
              `).join('') : ''}
            </ul>
          </div>
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Load content based on which page we're on
  // Check for directories instead of specific files
  if (path.includes('/blogs/') && !path.includes('/blogs/blog')) {
  ...
  } else if (!path.includes('/blogs/blog')) {
    // Fallback to Home, but exclude blog posts which have their own static content
